...
//		initFlashMapManager(context); // flash属性管理
	...
	private void initHandlerMappings(WebApplicationContext context) {
		...
		for(Entry<String, Object> entry : controllerMap.entrySet()) {
			...
			Object controller = AopProxyUtils.getTargetClass(proxy);
		...
		this.handlerMappings.stream().forEach(hm -> {
			System.out.printf(
				String.format("***Mapping: %s -> %s\n", hm.getUrlPattern().pattern(), hm.getHandler().toString())
			);
		});
	}
