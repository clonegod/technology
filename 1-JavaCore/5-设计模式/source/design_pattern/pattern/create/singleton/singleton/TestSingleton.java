...
public class TestSingleton {
	...
	public static void main(String[] args) throws Exception {
		...
		System.out.println("3. 懒汉式单例");
		doTest(tpe, () -> {
			objSet.add(LazyMan.getInstance().hashCode());
			cdl.countDown();
		});
		
		System.out.println("4. 枚举实现单例对象---注册式单例的一种实现形式");
		doTest(tpe, () -> {
			objSet.add(RegisterManEnum.BLACK.hashCode());
			cdl.countDown();
		});
		
		System.out.println("5. 单例对象的注册容器---注册式单例");
		doTest(tpe, () -> {
			objSet.add(RegisterMan.getInstance(Object.class.getName()).hashCode());
			cdl.countDown();
		});
		
		System.out.println("6. 保证单例对象在序列化之后再进行反序列化，得到的仍然是唯一的那个单例对象");
